...
- Request body fields destructured:
  - `event`, `status`, `customer_email`, `customer_name`, `plan_slug`, `plan_id`, `external_subscription_id`, `subscription_id`, `order_id`, `wp_user_id`, `subscription_status`, plus optional `valid_until`/`validUntil`.
- Required for activation events:
  - At least one identifier: `wp_user_id` **or** `customer_email` **or** `subscription_id` **or** `external_subscription_id` (otherwise 400 `missing_identifier`).
...
- Subscription identifier expectation:
  - Handler normalizes `subscriptionId = subscription_id || external_subscription_id || null`; same value passed to provisioning and used for existing-key lookup.
...
- Insert path (no existing key):
  - Generates new API key; inserts with columns `key_prefix`, `key_hash`, `key_last4`, `status='active'`, `plan_id`, `customer_email` (lowercased), `customer_name`, `wp_user_id`, `subscription_status`, `subscription_id`, `external_subscription_id`, `order_id`, `valid_from`, `valid_until`, timestamps.
- Update path (existing key):
  - Always sets `status='active'`, `plan_id`, `customer_email` (lowercased), `customer_name`, `wp_user_id`, `subscription_status`, `subscription_id`, `external_subscription_id`, `order_id`, `updated_at`, `license_key=NULL`; optionally updates validity window; regenerates key if missing hash.
...
- Request blocked before provisioning:
  ...
  - Missing all identifiers (`wp_user_id`, `customer_email`, `subscription_id`, `external_subscription_id`) → 400.
...
4. **Route branch**
   - **Activation events** (`activated|renewed|active|reactivated`):
     ...
     4. **Find existing key** (`findExistingKey`) → search by `wp_user_id` → `LOWER(customer_email)` → `subscription_id` → `external_subscription_id`.
...
- **Missing identifiers**: None of `wp_user_id`/`customer_email`/`subscription_id`/`external_subscription_id` provided → 400 `missing_identifier`.
