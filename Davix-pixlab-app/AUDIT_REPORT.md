...
- **POST /internal/subscription/event** (subscription-route, bridge-token): body `{ event/status, customer_email?, plan_slug?, plan_id?, subscription_id?, order_id? }` (legacy payloads with an external subscription id are accepted as aliases for `subscription_id` input-only); dispatches to activation/renew/disable via `activateOrProvisionKey` or `disableCustomerKey`; responds `{status:'ok', action:'created|updated|disabled', key?, key_prefix, plan_id, subscription_id, affected?}`.
